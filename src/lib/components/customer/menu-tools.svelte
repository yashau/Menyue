<script lang="ts">
	...
	let { categories, search = $bindable(''), searchLabel = 'Search menu', controls, children }: Props = $props();
	let activeCategory = $state<string | undefined>();
	let categoryObserver: IntersectionObserver | undefined;
	const isCurrent = (category: Category) => activeCategory === category.id || (!activeCategory && category.id === categories[0]?.id);
	...
	$effect(() => {
		...
		return () => { cancelAnimationFrame(frame); categoryObserver?.disconnect(); };
	});
</script>
...
<div class="customer-menu-layout" data-testid="customer-menu-tools">
		...
		<div class="customer-menu-controls">
			<div class="customer-menu-search" role="search">
				<label>
					...
					<input bind:value={search} aria-label={searchLabel} placeholder="Search dishes, drinks, allergies…" />
				</label>
	...
	<div class="customer-menu-content">{@render children?.()}</div>
</div>
...
<style>
	...
	@media (max-width: 760px) {
		...
		.customer-category-rail { display: flex; width: 100%; overflow-x: auto; scrollbar-width: none; }
		.customer-category-rail a { flex: none; border-radius: 99px; }
	}
</style>
